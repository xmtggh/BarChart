...
public class Tooltip extends RelativeLayout {
	 ...
	 */
	public void prepare(RectF rect, float value) {

		// If no previous dimensions defined, the size of the area of the entry will be used.
		int width = (mWidth == -1) ? (int) rect.width() : mWidth;
		int height = (mHeight == -1) ? (int) rect.height() : mHeight;
		...
		if (mHorizontalAlignment == Alignment.RIGHT_LEFT)
			layoutParams.leftMargin = (int) (rect.left - width - mRightMargin);
		if (mHorizontalAlignment == Alignment.LEFT_LEFT)
			layoutParams.leftMargin = (int) (rect.left + mLeftMargin);
		if (mHorizontalAlignment == Alignment.CENTER)
			layoutParams.leftMargin = (int) (rect.centerX() - width / 2);
		if (mHorizontalAlignment == Alignment.RIGHT_RIGHT)
			layoutParams.leftMargin = (int) (rect.right - width - mRightMargin);
		if (mHorizontalAlignment == Alignment.LEFT_RIGHT)
			layoutParams.leftMargin = (int) (rect.right + mLeftMargin);

		// Adjust top coordinate of tooltip based on the Alignment defined
		if (mVerticalAlignment == Alignment.BOTTOM_TOP)
			layoutParams.topMargin = (int) (rect.top - height - mBottomMargin);
		else if (mVerticalAlignment == Alignment.TOP_TOP)
			layoutParams.topMargin = (int) (rect.top + mTopMargin);
		else if (mVerticalAlignment == Alignment.CENTER)
			layoutParams.topMargin = (int) (rect.centerY() - height / 2);
		else if (mVerticalAlignment == Alignment.BOTTOM_BOTTOM)
			layoutParams.topMargin = (int) (rect.bottom - height - mBottomMargin);
		else if (mVerticalAlignment == Alignment.TOP_BOTTOM)
			layoutParams.topMargin = (int) (rect.bottom + mTopMargin);
